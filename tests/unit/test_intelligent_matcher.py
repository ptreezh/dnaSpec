...
from src.dnaspec_spec_kit_integration.core.matcher import IntelligentMatcher, MatchResult
...
class TestIntelligentMatcher:
    ...
    def test_skill_keywords_registration(self):
        ...
        result = matcher.register_skill_keywords('dnaspec-architect', keywords)
        assert result is True
        assert 'dnaspec-architect' in matcher._skill_keywords
        assert matcher._skill_keywords['dnaspec-architect'] == keywords
        
        # 测试无效注册
        result = matcher.register_skill_keywords('', keywords)
        assert result is False
        
        result = matcher.register_skill_keywords('dnaspec-test', [])
        assert result is False
    ...
    def test_command_pattern_registration(self):
        ...
        # 注册命令模式
        patterns = ['/speckit.dnaspec.architect', '/architect']
        result = matcher.register_command_pattern('dnaspec-architect', patterns)
        assert result is True
        assert 'dnaspec-architect' in matcher._command_patterns
        assert matcher._command_patterns['dnaspec-architect'] == patterns
    ...
    def test_spec_kit_command_matching(self):
        ...
        # 测试有效的spec.kit命令
        result = matcher._match_spec_kit_commands("/speckit.dnaspec.architect 设计电商系统")
    ...
    def test_keyword_matching(self):
        ...
        # 注册技能关键词
        matcher.register_skill_keywords('dnaspec-architect', ['架构', '系统设计', 'architecture'])
        matcher.register_skill_keywords('dnaspec-agent-creator', ['智能体', 'agent', 'create'])
        
        # 测试关键词匹配
        matches = matcher._find_keyword_matches("设计系统架构")
        assert len(matches) > 0
        assert any(match['skill_name'] == 'dnaspec-architect' for match in matches)
    ...
    def test_context_matching(self):
        ...
        # 注册技能关键词
        matcher.register_skill_keywords('dnaspec-architect', ['架构', '系统设计'])
    ...
    def test_intelligent_matching(self):
        ...
        # 测试spec.kit命令高优先级匹配
        result = matcher.match_intelligently("/speckit.dnaspec.architect 电商系统")
        assert result is not None
        assert result.skill_name == "dnaspec-architect"
        assert result.confidence > 0.9
    ...
    def test_matcher_info(self):
        ...
        # 注册一些数据
        matcher.register_skill_keywords('dnaspec-architect', ['架构'])
        matcher.register_command_pattern('dnaspec-architect', ['/speckit.dnaspec.architect'])
