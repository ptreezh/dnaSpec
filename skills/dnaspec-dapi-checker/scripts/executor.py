...
from .validator import DAPICheckerValidator
from .calculator import DAPICheckerCalculator
from .analyzer import DAPICheckerAnalyzer

class DAPICheckerExecutor:
    def __init__(self, skill_dir: Optional[Path] = None):
        if skill_dir is None:
            skill_dir = Path(__file__).parent.parent
        self.skill_dir = Path(skill_dir)
        self.prompts_dir = self.skill_dir / 'prompts'
        self.validator = DAPICheckerValidator()
        self.calculator = DAPICheckerCalculator()
        self.analyzer = DAPICheckerAnalyzer()
    ...
    def execute(self, request: str, context: Optional[Dict] = None, force_level: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {'success': True, 'validation': validation, 'metrics': metrics, 'analysis': analysis, 'prompt_level': level, 'prompt_content': prompt, 'recommendations': []}
