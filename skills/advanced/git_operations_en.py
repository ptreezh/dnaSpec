...
class GitOperationsSkill(BaseSkill):
    """Git Operations Skill - Provides complete Git operations support"""
    
    def __init__(self):
        super().__init__(
            name="git-operations",
            description="Provides complete Git operations support, including branch management, worktree, concurrent commits and CI/CD related functions"
        )
    ...
    def _execute_skill_logic(self, input_text: str, detail_level: DetailLevel, 
        ...
        # If no operation specified, return error
        if not operation:
            return {
                "success": False,
