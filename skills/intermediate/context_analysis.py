...
class ContextAnalysisSkill(BaseSkill):
    ...
    def __init__(self):
        super().__init__(
            ...
            description="Analyze context quality, provide assessment and improvement suggestions in five dimensions"
        )
    ...
    def _execute_skill_logic(self, input_text: str, detail_level: DetailLevel,
        ...
        suggestions = []
        if clarity < 0.7:
            suggestions.append("Increase more explicit terminology and goal statements")
        if completeness < 0.6:
        ...
        if "也许" in input_text or "可能" in input_text or "大概" in input_text:
            issues.append("Contains uncertain terminology: '也许'、'可能'、'大概'")
        if len(input_text) < 20:
            issues.append("Context too short, insufficient information")
        if "但是" in input_text and "因此" not in input_text:
            issues.append("Contains transition but lacks conclusion logic")
    ...
    def _format_output(self, result_data: Dict[str, Any], detail_level: DetailLevel) -> Dict[str, Any]:
        ...
        if detail_level == DetailLevel.BASIC:
            ...
            return {
                ...
                "main_issues": result_data["issues"][:3]  # Return only first 3 main issues
            }
            ...
            return {
                ...
                "suggestions": result_data["suggestions"][:5]  # Return only first 5 suggestions
            }
        else:  # DETAILED
            ...
            detailed_analysis = {
                ...
                "metrics_breakdown": {
                    "clarity": {
                        ...
                        "analysis": "Clarity is calculated based on explicit instruction words and sentence structure"
                    },
                    "relevance": {
                        "score": result_data["metrics"]["relevance"],
                        "analysis": "Relevance is calculated based on task-related vocabulary"
                    },
                    "completeness": {
                        "score": result_data["metrics"]["completeness"],
                        "analysis": "Completeness is calculated based on constraints and requirement vocabulary"
                    },
                    "consistency": {
                        "score": result_data["metrics"]["consistency"],
                        "analysis": "Consistency is calculated based on logical contradiction detection"
                    },
                    "efficiency": {
                        "score": result_data["metrics"]["efficiency"],
                        "analysis": "Efficiency is calculated based on information density"
                    }
                }
            }
    ...
    def _analyze_consistency(self, context: str) -> float:
        ...
        # Higher consistency scores are better, more contradictions score lower
    ...
    def _analyze_efficiency(self, context: str) -> float:
        ...
        # Normalize to 0-1 range (assuming ideal of 25 effective words per 100 characters is full score)
