...
│
├── context_analysis_skill.py        # Context analysis skill
├── context_optimization_skill.py    # Context optimization skill
├── cognitive_template_skill.py      # Cognitive template application skill
├── SKILLS_OVERVIEW.md               # Skills overview documentation
│
...
我们引入了专门的技能来增强上下文工程能力，支持更优化的项目分解和AI Agentic架构系统的上下文管理。

### 核心技能

1. **Context Analysis Skill** (`context_analysis_skill.py`)
   - 分析上下文的有效性
   - 评估各项指标（清晰度、相关性、完整性等）
   - 提供优化建议

2. **Context Optimization Skill** (`context_optimization_skill.py`) 
   - 基于分析结果优化上下文质量
   - 改进清晰度、相关性、完整性和简洁性
   - 自动化上下文改进流程

3. **Cognitive Template Skill** (`cognitive_template_skill.py`)
   - 应用认知模板到上下文工程任务
   - 提供思维链、少示例学习等框架
   - 结构化复杂推理过程
...
4. **Context Engineering Skills Manager** (`src/context_engineering_skills/skills_manager.py`)
   - 统一管理所有上下文工程技能
   - 提供协调和流水线功能

5. **Context Engineering Enhancement System** (`src/context_engineering_skills/system.py`)
   - 综合的上下文工程解决方案
   - 支持项目分解和AI Agentic架构的上下文管理
   - 提供完整的上下文审计功能
