...
def main():
    ...
    slash_parser.add_argument('skill_command', nargs='*', help='Skill command to execute (skill_name and parameters)')
    slash_parser.add_argument('--help-all', action='store_true', help='Show help for all available skills')
    
    # 手动解析参数以处理复杂的slash命令
    # 对于slash命令，我们需要特殊的参数解析
    if len(sys.argv) > 2 and sys.argv[1] == 'slash':
        # 找到第一个不是空格的参数
        skill_args = []
        for arg in sys.argv[2:]:
            if not arg.startswith('-'):
                skill_args.append(arg)
            else:
                break
    ...
    if args.command == 'exec':
        ...
        if not args.skill_command:
            print('❌ 请指定技能名称')
            print('用法: dnaspec slash <技能名> [参数]')
            print('例如: dnaspec slash context-analysis "待分析文本"')
            sys.exit(1)
        
        # 技能名称和参数 - args.skill_command现在是列表
        skill_parts = args.skill_command
        skill_name = skill_parts[0] if skill_parts else None
        skill_params = skill_parts[1:] if len(skill_parts) > 1 else []
        ...
        try:
            ...
            # 执行技能
            params_str = ' '.join(skill_params) if skill_params else ''
            result = skill_executor.execute(skill_name, params_str)
