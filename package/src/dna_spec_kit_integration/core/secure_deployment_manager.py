...
class SecureDeploymentManager:
    ...
    def _generate_secure_skill_executor_code(self, skill: Dict[str, Any]) -> str:
        ...
        project_root = self.project_root.as_posix()  # 使用POSIX格式避免Windows路径问题
...
# DNASPEC应该已经安装到系统site-packages中
try:
    from dna_context_engineering.skills_system_final import {function_name}
except ImportError as e:
    print(f"Error: DNASPEC skills module not found.")
    print(f"Import error: {{e}}")
    print("Please ensure DNASPEC is installed: pip install dnaspec-context-engineering-skills")
    sys.exit(1)
