...
class CLIExtensionDeployer:
    ...
    def deploy_all(self) -> Dict[str, Any]:
        ...
        if self.deployment_mode == 'stigmergy':
            stigmergy_result = self._deploy_with_stigmergy()
        else:
            stigmergy_result = self._deploy_cli_extensions()
        
        # 调用RealSkillDeployer部署到各CLI的原生skills目录
        print("🔄 Deploying skills to native CLI directories...")
        try:
            from .real_skill_deployer import RealSkillDeployer
            real_deployer = RealSkillDeployer()
            native_result = real_deployer.deploy_skills_to_all_platforms()
            
            print(f"✅ Native CLI deployment completed!")
            print(f"📊 Successfully deployed to {native_result.get('successful_deployments', 0)}/{native_result.get('total_installed_platforms', 0)} platforms")
            
            # 合并部署结果
            stigmergy_result['native_deployment'] = native_result
        except Exception as e:
            print(f"❌ Native CLI deployment failed: {e}")
            stigmergy_result['native_deployment_error'] = str(e)
