...
def test_skill_processing():
    ...
    # 测试正常请求处理
    test_request = "Check interface consistency for the user management system"
...
def test_key_points_extraction():
    ...
    # 测试关键点提取
    test_cases = [
        ("Check API interface consistency", ["api_interface"]),
        ("Verify system level module interfaces", ["system_level", "module_level"]),
        ("Review component documentation", ["component_level", "documentation"]),
        ("Perform comprehensive interface check", ["consistency_check"])
    ]
...
def test_consistency_checking():
    ...
    # 测试一致性检查
    test_request = "Check interface consistency across all system components"
    context = {}
    result = skill.process_request(test_request, context)
    
    assert result["status"] == "completed", "处理状态应该正确"
    check_result = result["check_result"]
    assert "total_interfaces" in check_result, "检查结果应该包含总接口数"
    assert "issues_found" in check_result, "检查结果应该包含发现问题"
