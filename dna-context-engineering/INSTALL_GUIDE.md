# DNASPEC Context Engineering Skills - 安装和使用指南

## 概述

DNASPEC (Dynamic Specification Growth System) Context Engineering Skills系统，提供专业的上下文工程增强工具集。系统基于AI原生设计理念，通过标准化指令模板，实现上下文分析、优化和结构化功能，包含完整的上下文工程技能、Git操作技能和临时工作区管理系统，专门设计用于安全的AI辅助开发流程。
...
```bash
# 使用斜杠命令调用技能
/speckit.dnaspec.context-analysis "分析系统设计文档的质量"
/speckit.dnaspec.context-optimization "优化需求描述的清晰度"
...
### 1. 上下文工程技能
- **Context Analysis Skill** (`dnaspec-context-analysis`): 分析上下文的5个维度质量
- **Context Optimization Skill** (`dnaspec-context-optimization`): 基于分析结果优化上下文
- **Cognitive Template Skill** (`dnaspec-cognitive-template`): 应用认知模板到任务

### 2. Git操作技能
- **Git Skill** (`dnaspec-git-skill`): 完整的Git操作支持
...
### 3. 临时工作区管理技能
- **Temporary Workspace Skill** (`dnaspec-temp-workspace-skill`): AI安全工作流管理
...
- Claude CLI: 通过 `/speckit.dnaspec.*` 指令集成
