...
def run_comprehensive_verification():
    ...
    try:
        ...
        test_context = "开发一个电商平台，需要支持用户注册登录、商品浏览、购物车功能。"
        ...
        if result.status.name == 'COMPLETED':
            ...
            print("   ✅ Context Analysis 验证失败")
            verification_results['context_analysis'] = False
        ...
        if result.status.name == 'COMPLETED':
            ...
            if 'applied_optimizations' in result_data:
                ...
                print(f"   优化技能返回格式错误: {result.result}")
                verification_results['context_optimization'] = False
        else:
        ...
        if result.status.name == 'COMPLETED':
            ...
            template_result_data = result_data.get('result', result_data) if 'success' in result_data else result_data
            print(f"   认知模板应用: {template_result_data['template_type']}")
            print(f"   结构化结果长度: {len(template_result_data['enhanced_context'])} 字符")
...
def test_ai_native_characteristics():
    ...
    # 验证不包含本地机器学习库
    ml_indicators = ['sklearn', 'tensorflow', 'pytorch', 'torch', 'xgboost', 'lightgbm']
    has_local_ml = any(indicator in code_content.lower() for indicator in ml_indicators)
    
    print(f"   ✓ 无本地机器学习模型: {'✅' if not has_local_ml else '❌'}")
    
    # 验证使用指令工程模式
    instruction_indicators = ['ai instruction', 'send instruction', 'analysis instruction', 'optimization instruction']
    uses_instruction_eng = any(indicator in code_content.lower() for indicator in instruction_indicators)
    
    print(f"   ✓ 使用指令工程: {'✅' if uses_instruction_eng else '❌'}")
    
    # 验证AI原生智能利用
    ai_native_indicators = ['execute_with_ai', 'process_request', 'ai model native intelligence']
    uses_ai_native_approach = any(indicator in code_content.lower() for indicator in ai_native_indicators)
    
    print(f"   ✓ AI原生智能利用: {'✅' if uses_ai_native_approach else '❌'}")
    
    return not has_local_ml and uses_instruction_eng and uses_ai_native_approach
