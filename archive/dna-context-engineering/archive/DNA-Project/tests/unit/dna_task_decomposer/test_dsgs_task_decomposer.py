# dnaspec-task-decomposer子技能单元测试
...
def test_skill_metadata():
    """测试技能元数据定义"""
    # 测试SKILL.md文件存在
    skill_md_path = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'skills', 'dnaspec-task-decomposer', 'SKILL.md')
    ...
    # 验证必需字段
    assert 'name: dnaspec-task-decomposer' in content, "应该包含正确的技能名称"
...
def test_skill_basic_functionality():
    ...
    try:
        ...
        skill = DNASPECTaskDecomposer()
        assert skill is not None, "技能实例不应该为空"
        assert skill.name == "dnaspec-task-decomposer", "技能名称应该正确"
...
def test_skill_processing():
    ...
    # 测试正常请求处理
    test_request = "Decompose tasks for developing a web application with API backend"
    result = skill.process_request(test_request)
    
    assert result["status"] == "completed", "处理状态应该正确"
    assert result["skill"] == "dnaspec-task-decomposer", "技能名称应该正确"
    assert result["request"] == test_request, "请求内容应该正确"
    assert "task_decomposition" in result, "应该包含任务分解结果"
...
def test_key_points_extraction():
    ...
    # 测试关键点提取
    test_cases = [
        ("Develop a web application with database integration", ["web_development", "data_management"]),
        ("Create a mobile app with API services", ["mobile_app", "api_development"]),
        ("Build a secure system with testing", ["security_implementation", "testing"]),
...
def test_atomic_task_creation():
    ...
    # 测试原子任务创建
    key_points = ["web_development", "api_development"]
    tasks = skill._create_atomic_tasks("Develop a web application with API", key_points)
    
    assert len(tasks) > 0, "应该创建至少一个任务"
    for task in tasks:
        assert "id" in task, "每个任务应该有ID"
        assert "name" in task, "每个任务应该有名称"
        assert "description" in task, "每个任务应该有描述"
        assert "type" in task, "每个任务应该有类型"
        assert "estimated_hours" in task, "每个任务应该有预估工时"

if __name__ == "__main__":
    # 简单测试执行
    print("Running dnaspec-task-decomposer tests...")
    ...
    try:
        test_atomic_task_creation()
        print("✓ test_atomic_task_creation passed")
    except Exception as e:
        print(f"✗ test_atomic_task_creation failed: {e}")
