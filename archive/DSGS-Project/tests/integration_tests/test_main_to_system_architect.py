...
def test_integration():
    ...
    assert routed_skill == "dnaspec-system-architect", f"请求应该路由到dsgs-system-architect，但实际路由到{routed_skill}"
    print("✓ 主技能路由测试通过")
    
    # 2. 验证系统架构师子技能能正确处理请求
    result = system_architect_skill.process_request(test_request)
    assert result["status"] == "completed", "子技能处理状态应该为completed"
    assert result["skill"] == "dnaspec-system-architect", "处理技能应该是dsgs-system-architect"
    ...
    assert main_result["skill_used"] == "dnaspec-system-architect", "使用的技能应该是dsgs-system-architect"
    print("✓ 完整流程测试通过")
