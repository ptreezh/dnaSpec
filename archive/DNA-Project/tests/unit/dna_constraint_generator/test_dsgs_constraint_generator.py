# dnaspec-constraint-generator子技能单元测试
...
def test_skill_metadata():
    """测试技能元数据定义"""
    # 测试SKILL.md文件存在
    skill_md_path = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'skills', 'dnaspec-constraint-generator', 'SKILL.md')
    ...
    # 验证必需字段
    assert 'name: dnaspec-constraint-generator' in content, "应该包含正确的技能名称"
...
def test_skill_basic_functionality():
    ...
    try:
        ...
        skill = DNASPECConstraintGenerator()
        assert skill is not None, "技能实例不应该为空"
        assert skill.name == "dnaspec-constraint-generator", "技能名称应该正确"
...
def test_skill_processing():
    ...
    # 测试正常请求处理
    test_request = "Generate constraints for developing a web application with API services"
    result = skill.process_request(test_request)
    
    assert result["status"] == "completed", "处理状态应该正确"
    assert result["skill"] == "dnaspec-constraint-generator", "技能名称应该正确"
    assert result["request"] == test_request, "请求内容应该正确"
    assert "constraint_specification" in result, "应该包含约束规范结果"
...
def test_key_points_extraction():
    ...
    # 测试关键点提取
    test_cases = [
        ("Generate constraints for web application development", ["web_application"]),
        ("Create API constraints for service", ["api_service"]),
        ("Define data and security constraints", ["data_processing", "security_constraint"]),
        ("Generate general constraints for project", ["general_constraint"])
    ]
...
def test_constraint_generation():
    ...
    constraints = skill._generate_constraint_specification(test_request, {"key_points": key_points})
    
    assert "system_constraints" in constraints, "应该包含系统约束"
    assert "api_constraints" in constraints, "应该包含API约束"
    assert "data_constraints" in constraints, "应该包含数据约束"
    assert "quality_constraints" in constraints, "应该包含质量约束"
    assert "validation_mechanisms" in constraints, "应该包含验证机制"
    
    assert len(constraints["system_constraints"]) >= 0, "系统约束列表应该存在"
    assert len(constraints["api_constraints"]) >= 0, "API约束列表应该存在"
    assert len(constraints["data_constraints"]) >= 0, "数据约束列表应该存在"

if __name__ == "__main__":
    # 简单测试执行
    print("Running dnaspec-constraint-generator tests...")
    ...
    try:
        ...
        print("✓ test_constraint_generation passed")
    except Exception as e:
        print(f"✗ test_constraint_generation failed: {e}")
