...
class ContextAnalysisSkill(DNASpecSkill):
    ...
    def _execute_skill_logic(self, context: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        """执行上下文分析"""
...
class CognitiveTemplateSkill(DNASpecSkill):
    ...
    def _execute_skill_logic(self, context: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if template_type not in self.templates:
            return {
                'success': False,
                'error': f'未知模板: {template_type}. 支持: {list(self.templates.keys())}',
                'result': {'success': False}
            }
...
"""
        ...
        try:
            ...
            # 模拟AI模板处理
            enhanced_content = f"""
### {template_type} 认知模板应用 ({template_desc})
        ...
        if skill_name == 'context-analysis':
            ...
            if result.status.name == 'COMPLETED':
                ...
                for metric, score in analysis['metrics'].items():
                    names = {'clarity': '清晰度', 'relevance': '相关性', 'completeness': '完整性', 
            ...
            if result.status.name == 'COMPLETED' and result.result['result']['success']:
                ...
                error_msg = result.result['result'].get('error', '模板应用失败') if result.status.name == 'COMPLETED' else result.error_message
