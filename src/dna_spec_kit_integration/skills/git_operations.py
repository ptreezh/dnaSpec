...
用于项目初始化时设置Git规则和项目宪法，避免AI生成文件污染工作区
"""
...
def execute(args: Dict[str, Any]) -> str:
    ...
    if operation == "setup-constitution":
        ...
        return validate_commit_message(project_path, args.get("message", ""))
    elif operation == "smart-commit":
        return smart_commit(project_path, args.get("message", ""))
        ...
        return create_workflow_rules(project_path, args.get("workflow", {}))
    elif operation == "enforce-rules":
        return enforce_git_rules(project_path)
    elif operation == "branch-policy":
        return setup_branch_policy(project_path, args.get("policy", {}))
    elif operation == "review-policy":
        return setup_review_policy(project_path, args.get("review_config", {}))
    else:
...
def smart_commit(project_path: str, commit_message: str = "") -> str:
    ...
    try:
        ...
        # 执行提交前验证
        validation = validate_commit_message(project_path, commit_message)
        if "❌" in validation:
            return validation
...
def generate_smart_commit_message(changes: List) -> str:
    ...
    return f"{commit_type}(workspace): {description}"
